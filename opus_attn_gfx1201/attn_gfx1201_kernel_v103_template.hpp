// v103 -- v100 (v63 K-prefetch + v43 batched pre-PV rescale) + a LATE, SINGLE-FRAGMENT
// dt=0 V hoist. Reviewer feedback on v102: hoisting BOTH dt=0 V fragments right after
// QKT carried two bf16x8 V operands across the ENTIRE softmax window (max-reduce, cross-
// half bpermute, conditional O rescale, 16 exp2, sum-reduce, bf16 pack) -> inflated VGPR
// liveness past an occupancy/scheduling threshold and lost to v100 (86.64 < 88.07).
// v103 narrows the idea on BOTH axes the reviewer suggested:
//   1. LATE placement: issue the V load AFTER the sum-reduce, immediately before the
//      bf16 P packing. The load latency now overlaps only with the bf16 RNE pack (16
//      VALU ops) and the dt=0 PV WMMA setup -- a short live range, not the whole
//      softmax. Hides the first PV-load bubble without the VGPR tax.
//   2. SINGLE fragment: hoist only v_v0_first (operand of the FIRST dt=0 PV WMMA, the
//      actual head-of-chain bubble). v_v1_first stays an in-loop load, so we add one
//      bf16x8 of liveness instead of two -- half the pressure of v102.
// Everything else is identical to v100 (the 88.07 best).
...
__global__ void opus_attn_gfx1201_kernel_v103(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        // ---- LATE single-fragment dt=0 V hoist (v103) ----
        // Issue the head-of-chain V load now so its global latency overlaps the bf16
        // pack below + the first PV WMMA setup. Short live range (only one bf16x8,
        // alive ~16 VALU ops) -- avoids the whole-softmax VGPR tax that sank v102.
        const bf16_t* vt_addr0_first = VTp + col16 * vt_stride_d + n_base + row8;
        bf16x8_t v_v0_first = *reinterpret_cast<const bf16x8_t*>(vt_addr0_first);
        ...
        // dt=0's first operand is the hoisted v_v0_first; all other loads stay in-loop.
        #pragma unroll
        for (int dt = 0; dt < DK; ++dt) {
            ...
            if (dt == 0) {
                ...
                v_v0 = *reinterpret_cast<const bf16x8_t*>(vt_addr0);
            }
