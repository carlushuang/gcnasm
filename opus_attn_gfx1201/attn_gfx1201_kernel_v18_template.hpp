...
// opus_attn_gfx1201 v18 — BLOCK_N=64 + bf16_fast
//
// Combines v17's BLOCK_N=64 (4 sub-tiles) with v15's bf16_fast truncation.
// BLOCK_N=64 provides enough instruction volume between loads and uses
// to hide latency, even with the reduced bf16 conversion overhead.
//
//   workgroup = 1 wave x 32 lanes, BLOCK_M=16, BLOCK_N=64, D=128
...
__device__ static inline bf16_t bf16_fast18(fp32_t f) {
    unsigned int x = __builtin_bit_cast(unsigned int, f);
    x += 0x7FFF + ((x >> 16) & 1);
    return static_cast<bf16_t>(x >> 16);
}

__device__ static inline fp32_t v18_fmaxf(fp32_t a, fp32_t b) { return a > b ? a : b; }

__device__ static inline fp32_t v18_cross_half_max(fp32_t v) {
    int x = __builtin_bit_cast(int, v);
    int other = __builtin_amdgcn_ds_bpermute((threadIdx.x ^ 16) << 2, x);
    return v18_fmaxf(v, __builtin_bit_cast(fp32_t, other));
}

__device__ static inline fp32_t v18_cross_half_sum(fp32_t v) {
    int x = __builtin_bit_cast(int, v);
    int other = __builtin_amdgcn_ds_bpermute((threadIdx.x ^ 16) << 2, x);
...
__global__ void opus_attn_gfx1201_kernel_v18(opus_attn_kargs k)
{
    ...
    constexpr int N_SUB   = BLOCK_N / 16;  // 4
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        for (int ns = 0; ns < N_SUB; ++ns) {
            ...
            for (int j = (ns == 0 ? 1 : 0); j < 8; ++j)
                row_max = v18_fmaxf(row_max, v_s[ns][j]);
        }
        row_max = v18_cross_half_max(row_max);

        const fp32_t new_m = v18_fmaxf(m_row, row_max);
        ...
        row_sum = v18_cross_half_sum(row_sum);
        ...
        for (int ns = 0; ns < N_SUB; ++ns) {
            bf16x8_t v_p;
            #pragma unroll
            for (int j = 0; j < 8; ++j) v_p[j] = bf16_fast18(v_s[ns][j]);
    ...
    for (int dt = 0; dt < DK; ++dt) {
        bf16x8_t o_pack;
        #pragma unroll
        for (int j = 0; j < 8; ++j) o_pack[j] = bf16_fast18(v_o[dt][j] * inv);
