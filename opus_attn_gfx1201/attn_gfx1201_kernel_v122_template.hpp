// v122 -- DIRECTOR DIRECTION (A): recover v111's measured 91.59 TFLOPS schedule
// while making it BIT-EXACT versus v100 (max_abs ~0.0001, not 0.036).
//
// Diff-confirmed fact: the ONLY arithmetic-touching change v100 -> v111 is the
// rescale restructuring. v100 applies the online-softmax rescale to all 8 O
// accumulators in one monolithic pre-PV pass; v111 splits it into CHUNK=2 groups
// interleaved with the PV WMMA loop. The PER-ELEMENT multiply order is IDENTICAL
// (dt-major, j-minor) and rescale of v_o[dt] is still strictly before that dt's PV
// WMMAs -- so the two are algebraically equivalent. The faster chunked SCHEDULE
// (only 2 O fragments hot per VALU region, rescale of chunk c+1 hides in chunk c's
// matrix-pipe WMMA shadow, V loads issue right after each chunk's rescale) is the
// proven speed lever and is kept verbatim.
//
// WHY v111 lost bit-exactness (and how v122 fixes it): hipcc defaults to
// -ffp-contract=fast. In v100 the rescale multiply lives in an isolated VALU loop,
// far from any WMMA, so the backend cannot contract/reassociate it. In v111 the
// rescale multiply sits ADJACENT to the PV WMMA in the same scheduling region,
// which lets the fast-contract/reassoc machinery alter the rounding of the v_o
// values that feed the next WMMA -> the benign max_abs~0.036 drift. v122 pins
// FP contraction OFF (#pragma clang fp contract(off)) over the chunked rescale +
// PV region, forcing the interleaved code to round EXACTLY as v100's monolithic
// loop does, while the compiler is still free to overlap the (now strictly
// non-contracted) multiplies with the WMMAs for the v111 issue schedule.
// EXPECTED: primary ~91-92 TFLOPS (recover v111) with max_abs<=0.0005, n_bad=0.
// RISK: low -- contract(off) cannot worsen perf vs v100 (v100 already isn't
// contracting the rescale); worst case it matches v100's 88.07 bit-exactly.
...
__global__ void opus_attn_gfx1201_kernel_v122(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        if (need_rescale) {
            ...
            // one VALU block). v122 defers it into 2-tile chunks interleaved with
        ...
        // ---- PV: chunked rescale interleaved with WMMAs (v122) ----
        ...
        // Pin FP contraction OFF over the interleaved rescale+PV region. This is the
        // ONLY substantive change vs v111: it forces the rescale multiply (now
        // adjacent to the PV WMMAs) to round EXACTLY as v100's isolated monolithic
        // rescale loop does, restoring bit-exactness without disturbing the faster
        // v111 issue schedule. An IEEE multiply is rounding-deterministic regardless
        // of schedule, so v111's max_abs~0.036 can only stem from contract/reassoc
        // that the new adjacency unlocked -- contract(off) removes exactly that.
        {
