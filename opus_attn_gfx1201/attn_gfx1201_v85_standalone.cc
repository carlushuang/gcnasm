// v85: BLOCK_N=64 via HIP with generalized N_COLS
...
#include "attn_gfx1201_kernel_v85_template.hpp"
...
int main(int argc, char** argv) {
    ...
    for (int i=1;i<argc;++i) { const char*a=argv[i];
        ...
        if(eq("--block_m",block_m))continue; if(eq("--block_n",block_n))continue;
    }
    ...
    auto run = [&](auto traits_tag) {
        ...
        printf("running version v85-blockM%d-blockN%d  B=%d H=%d N=%d D=%d\n",Traits::BLOCK_M,Traits::BLOCK_N,B,H,N,D);
        auto kern = opus_attn_gfx1201_kernel_v85<Traits>;
    ...
    // BLOCK_N=32 configs (like v66)
    if (block_n == 32) {
        if (block_m == 128) run(opus_attn_traits_v85<128,32,128>{});
        else if (block_m == 256) run(opus_attn_traits_v85<256,32,128>{});
        else if (block_m == 384) run(opus_attn_traits_v85<384,32,128>{});
        else if (block_m == 448) run(opus_attn_traits_v85<448,32,128>{});
        else { printf("unsupported block_m=%d for block_n=32\n", block_m); return 1; }
    }
    // BLOCK_N=64 configs
    else if (block_n == 64) {
        if (block_m == 128) run(opus_attn_traits_v85<128,64,128>{});
        else if (block_m == 192) run(opus_attn_traits_v85<192,64,128>{});
        else if (block_m == 256) run(opus_attn_traits_v85<256,64,128>{});
        else if (block_m == 320) run(opus_attn_traits_v85<320,64,128>{});
        else if (block_m == 384) run(opus_attn_traits_v85<384,64,128>{});
        else if (block_m == 448) run(opus_attn_traits_v85<448,64,128>{});
        else { printf("unsupported block_m=%d for block_n=64\n", block_m); return 1; }
    }
    else { printf("unsupported block_n=%d\n", block_n); return 1; }
