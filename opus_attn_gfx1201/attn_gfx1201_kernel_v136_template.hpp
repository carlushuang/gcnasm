// v136 -- v132 champion (92.78 TFLOPS, bit-exact) with ONE surgical change:
// inside the chunked PV loop, the per-chunk O-accumulator RESCALE (pure VALU)
// is demoted to prio 0 with a COARSE transition per CHUNK=2 feeder block, while
// the chunk's V loads + PV WMMA pairs stay at prio 2. ROUND-19 (lane B).
//
// ================= WHY (round-19 reviewer NEXT lever #1) ===================
// History this round:
//   v132 (champion, 92.78): chunked rescale/PV overlap, but the rescale FMAs run
//        at prio 2 *inside* the PV burst -> short VALU regions punctuate the
//        prio-2 matrix arbitration (a known wart; v134 showed extra prio-2 over
//        feeder VALU is neutral-to-bad).
//   v135 (FAIL, 91.8): tried to make PV a "pure" matrix burst by hoisting ALL 8
//        fragments' rescale into one monolithic prio-0 block BEFORE PV. That
//        lengthened the pre-PV critical path: the first PV WMMA could not issue
//        until all 8 O fragments were rescaled. Lost the early-PV-start overlap
//        that is v132's whole point. Verdict: monolithic placement is a dead end.
//
// The reviewer's NEXT FOCUS (verbatim): "Restore v132's chunked rescale/PV
// overlap, and test moving only the rescale chunks to prio0 with coarse CHUNK=2
// priority boundaries." v136 is exactly that:
//   - KEEP v132's chunked placement (rescale chunk c emitted immediately before
//     chunk c's PV WMMAs) -> first PV WMMA still issues after only CHUNK=2
//     fragments are rescaled. Early-PV-start preserved (unlike v135).
//   - DEMOTE only the rescale FMAs to prio 0 (yieldable feeder VALU) and RAISE
//     back to prio 2 for that chunk's V loads + WMMA pairs. So the prio-2 matrix
//     arbitration is no longer punctuated by rescale FMAs (the wart v132 had),
//     yet the rescale stays interleaved chunk-by-chunk (the overlap v135 lost).
//   - COARSE boundaries: exactly ONE prio toggle pair per CHUNK=2 block (2 toggles
//     per chunk, DK/2 chunks), NOT per WMMA. Lane A's v136-equiv proved per-WMMA
//     toggling inside a burst regresses; coarse per-chunk transitions amortize
//     the s_setprio cost over CHUNK*2 = 4 WMMAs.
//   When need_rescale is false (no rescale emitted) the loop stays at prio 2
//   uninterrupted, identical to v132 for that iteration.
//
// ================= ORIGINAL v132 RATIONALE (priority policy, kept) ==========
...
// v132 applied BOTH in one coherent change:
//
...
//       arbitration at the DEFAULT priority, not prio 2. v136 issues s_setprio(2)
...
// v136 MUST be in the host dVT (pre-transposed V, layout [B,H,D,N]) allow-list;
...
// kernel was always correct; only the wiring was wrong. Host registers v136 in
...
__global__ void opus_attn_gfx1201_kernel_v136(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        // v136: raise BEFORE the K preloads (v131 raised it after), so the FIRST
        ...
        // theory its 8-chain ILP lets it yield; v136 raises it to prio 2 (uniform
        ...
        // Uniform prio 2 on all WMMA pairs lets HW round-robin fairly. v136 keeps
        // every PV WMMA at prio 2; the ONLY prio-0 windows are the per-chunk
        // rescale FMA blocks (see below). The WMMA pairs themselves are never
        // punctuated by a toggle -- the demote/raise straddles only the pure-VALU
        // rescale, at COARSE CHUNK=2 granularity (not per WMMA, which regresses).
        __builtin_amdgcn_s_setprio(2);
        constexpr int CHUNK = 2;
        #pragma unroll
        for (int c0 = 0; c0 < DK; c0 += CHUNK) {
            if (need_rescale) {
                // v136: this chunk's rescale is pure feeder VALU. Demote to prio 0
                // so it never out-competes another resident wave's ready WMMA,
                // then raise back to prio 2 before this chunk's V loads + WMMAs.
                // COARSE: exactly one toggle pair per CHUNK=2 block (not per WMMA).
                __builtin_amdgcn_s_setprio(0);
                #pragma unroll
