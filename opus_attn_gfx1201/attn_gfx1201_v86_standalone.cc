...
#include "attn_gfx1201_kernel_v86_template.hpp"
...
int main(int argc, char** argv) {
    ...
    auto run = [&](auto traits_tag) {
        ...
        printf("running version v86-blockM%d  B=%d H=%d N=%d D=%d\n",Traits::BLOCK_M,B,H,N,D);
        auto kern = opus_attn_gfx1201_kernel_v86<Traits>;
    ...
    if (block_m == 128) run(opus_attn_traits<128,64,128>{});
    else if (block_m == 192) run(opus_attn_traits<192,64,128>{});
    else if (block_m == 256) run(opus_attn_traits<256,64,128>{});
    else if (block_m == 320) run(opus_attn_traits<320,64,128>{});
    else if (block_m == 384) run(opus_attn_traits<384,64,128>{});
    else if (block_m == 448) run(opus_attn_traits<448,64,128>{});
