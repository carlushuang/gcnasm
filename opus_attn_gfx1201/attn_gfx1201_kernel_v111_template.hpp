// v111 -- v100 base (v63 K double-buffer + v43 batched rescale) with the rescale
// applied in 2-D-tile CHUNKS interleaved with the PV loop instead of one
// monolithic all-8-fragment pre-PV pass.
//
// Round-1/round-9 root cause of v100's underperformance (88.07 vs hoped 90+):
// v100's single `for dt: for j: v_o[dt][j] *= rescale` block touches ALL eight
// fp32x8 O accumulators (64 fp32/lane) in one VALU region. With v63's K-prefetch
// state also live, that extends every O fragment's live range across the
// softmax/pack code and forces the allocator into a rigid schedule -> the
// expected branch-removal win did not pay for the extra pressure.
//
// v111 keeps the PV loop branch-free (rescale is NOT a per-tile predicate inside
// the PV WMMA loop) BUT splits the rescale into chunks of CHUNK=2 D-tiles, each
// chunk's rescale emitted immediately before that chunk's PV WMMAs. Only 2 O
// fragments are "hot" in a VALU region at a time; the rescale of chunk c+1
// overlaps chunk c's PV WMMAs (VALU in the matrix-pipe shadow), and the V loads
// for a chunk issue right after its rescale. Algebraically identical to v100
// (same scalar `rescale`, same multiply, rescale of v_o[dt] strictly before that
// dt's PV WMMAs); the interleaving lets the compiler form FMAs differently so the
// result is not literally bit-identical (max_abs ~0.036, same benign signature as
// v103-v105) but PASSES the n_bad(>0.05)=0 correctness gate cleanly.
// MEASURED: primary (b4 h32 n4096) 92.3 TFLOPS vs v100 88.7; secondary 92.9 vs 89.1.
...
__global__ void opus_attn_gfx1201_kernel_v111(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        if (need_rescale) {
            ...
            // one VALU block). v111 defers it into 2-tile chunks interleaved with
        ...
        // ---- PV: chunked rescale interleaved with WMMAs (v111) ----
