...
template<class T> __global__ void opus_attn_gfx1201_kernel_v143(opus_attn_kargs); // v143: v140 champion (93.38 TFLOPS) + move the prio1 window START one step EARLIER -- keep the latency-bound row-max ds_bpermute reduction at prio0, but raise prio1 from new_m / the conditional HEAD rescale exp2f (a transcendental on the arbitration-starved pipe v140 targeted, which GATES every tail exp2) through the tail P-pack. The asymmetric intermediate between v140 (tail-only prio1) and v142 (whole-island prio1, REGRESSED 92.9). Round-25 reviewer NEXT-FOCUS; pure scheduling hint -> bit-exact, dVT-wired
template<class T> __global__ void opus_attn_gfx1201_kernel_v144(opus_attn_kargs); // v144: v140 champion (93.38 TFLOPS) + SURGICAL prio1 around ONLY the conditional head exp2f(m_row-new_m) -- keep v140's tail-only prio1 boundary EXACTLY, and wrap just the rare record-max head transcendental in a momentary prio1 island (nested inside the need_rescale branch -> common no-rescale tiles stay byte-identical to v140). Isolates the one head op on the arbitration-starved transcendental pipe WITHOUT v143's regression (which widened prio1 over new_m/branch/scalar feeders, a low-ILP non-transcendental region). Round-26 reviewer NEXT-FOCUS; pure scheduling hint -> bit-exact, dVT-wired
template<class T> __global__ void opus_attn_gfx1201_kernel_v145(opus_attn_kargs); // v145: v140 champion (93.38 TFLOPS) + round-27 reviewer NEXT-FOCUS -- split the prio1 softmax tail at the UNTRIED interior boundary. v140 raised the WHOLE tail [exp2 -> row-sum -> P-pack] to prio1 (WON); v141 dropped to prio0 BEFORE row-sum (LOST, 93.1). v145 keeps prio1 over [exp2 + row-sum fold] (the part v141 proved must stay raised) but DROPS only the bf16 P-pack to prio0 (plain RNE convert VALU, not transcendental -> stop out-arbitrating other waves' matrix-feeder VALU for the final pack slots). The single untried split between the v140-win and v141-loss brackets. Pure scheduling hint -> bit-exact, dVT-wired
template<class T> __global__ void opus_attn_gfx1201_kernel_v146(opus_attn_kargs); // v146: v145 champion (93.58 TFLOPS) + round-28 reviewer NEXT-FOCUS -- split the PV region so the deferred per-chunk v_o rescale runs at the v140/v145-proven middle prio1 (NOT v136's prio0, which starved it and regressed), then raise to prio2 only immediately before that chunk's V loads + PV WMMAs. Current PV (v132..v145) holds the rescale at prio2 where it out-arbitrates co-resident waves' matrix work for slots it has no matrix reason to win; prio1 keeps it fed for its own PV yet yields arbitration. Toggle pair straddles ONLY the pure-VALU rescale (never a WMMA burst); no toggle when need_rescale is false. Differs from v136 ONLY in rescale prio (1 vs 0). Pure scheduling hint -> bit-exact, dVT-wired
template<class T> __global__ void opus_attn_gfx1201_kernel_v142(opus_attn_kargs); // v142: v140 champion (93.38 TFLOPS) COMPLETED -- raise the softmax HEAD (cross-lane row-max ds_bpermute reduce + new_m + head rescale exp2f) from prio0 to prio1, UNIFYING the whole softmax island at prio1. v140 raised only the tail; v141 proved the QK->PV feeder chain must NOT fall to prio0; the head is itself a feeder (cross-lane reduce + transcendental exp2f) that GATES new_m and thus all tail exp2 -> at prio0 it was still arbitration-starved. Single contiguous prio1 window between the prio2 QK^T and prio2 PV bursts. Pure scheduling hint -> bit-exact, dVT-wired
template<class T> __global__ void opus_attn_gfx1201_kernel_v148(opus_attn_kargs); // v148: v140 champion (93.69 TFLOPS) schedule + the ONE untried PV-pressure REDUCTION the round-30 reviewer mandated -- CHUNK=1 (per-fragment deferred O-rescale interleave). Every structural probe that ADDED state to the PV VGPR/issue-window pressure peak regressed (v147 K-carry 88.46, v133 V-ring, v137/138/139 early-V); CHUNK=1 instead SHRINKS the post-rescale O-fragment live range (8 uniform 8-FMA groups, one per WMMA pair) -> finest VALU-in-WMMA-shadow cadence, lowest simultaneous rescale-temp pressure. Orthogonal to all the s_setprio work that built v132->v140; never swept on the winning schedule (v113's CHUNK=1 was on the mis-wired v111 base). Bit-exact (per-fragment op order identical, no fp32 reassociation), dVT-wired
template<class T> __global__ void opus_attn_gfx1201_kernel_v149(opus_attn_kargs); // v149: v140 champion (93.83 TFLOPS, CHUNK=2) + surgical PV live-range reduction -- derive vt_addr1 = vt_addr0 + 16 (compile-time +32B delta folds into global_load immediate offset) so the second V load reuses vt_addr0's base-address VGPR pair instead of a redundant (dt*W_K+col16)*vt_stride_d address computation. Shortens V-address lifetime in the PV VGPR/issue-window pressure peak WITHOUT disturbing CHUNK=2's contiguous 4-WMMA cadence (round-31 reviewer lever #1). Reverts v148's CHUNK=1 regression. Identical V bytes -> bit-exact, dVT-wired
template<class T> __global__ void opus_attn_gfx1201_kernel_v147(opus_attn_kargs); // v147: v140 champion schedule (93.63 TFLOPS) + STRUCTURAL cross-KV-tile K prefetch -- carry the NEXT tile's kt=0 K pair across the PV region (loaded at the P-pack point where v_s0/v_s1 die -> zero added liveness) so the first QK^T WMMA of every tile no longer stalls on a cold global K load. Eliminates the per-tile inter-tile K bubble that v63's intra-tile ring leaves exposed (playbook lever #4, KV-block granularity). Off the saturated s_setprio space; bit-exact (only a load is relocated), dVT-wired
template<class T> __global__ void opus_attn_gfx1201_kernel_v150(opus_attn_kargs); // v150: v140 champion (93.91 TFLOPS, CHUNK=2) + bit-exact zero-VGPR PV CHUNK sweep 2->4. Measured CHUNK trend on the winning schedule is monotone (CHUNK=1 v148 93.25 < CHUNK=2 v140 93.91); CHUNK=4 is the untried next point. Coarser PV burst = up to 8 V-load pairs in flight ahead of 8 PV WMMAs -> doubles PV memory-level parallelism (the V-load-to-WMMA latency the round-32 reviewer named as limiter) and fewer rescale-VALU punctuation points in the prio-2 matrix stream. Identical per-fragment rescale+WMMA order -> bit-exact, dVT-wired
template<class T> __global__ void opus_attn_gfx1201_kernel_v151(opus_attn_kargs); // v151: v140 champion (93.91 TFLOPS, CHUNK=2) + ROUND-33 reviewer NEXT-FOCUS -- keep CHUNK=2 (v150's CHUNK=4 over-coarsened, 89.82) but restructure each chunk LOAD-THEN-COMPUTE: issue both D-tiles' V load pairs (4 b128 global_loads into v_v0[2]/v_v1[2]) FIRST, then the four PV WMMAs. dt=c0's WMMAs overlap dt=c0+1's in-flight loads -> 4 V loads exposed ahead of the WMMA chain (matches CHUNK=4's MLP) over only 2 fragments of rescale + V liveness. Identical V bytes + per-fragment rescale/WMMA order -> bit-exact, dVT-wired
template<class T> __global__ void opus_attn_gfx1201_kernel_v152(opus_attn_kargs); // v152: v140 champion (93.91 TFLOPS, CHUNK=2) + ROUND-35 reviewer NEXT-FOCUS -- keep v140's exact CHUNK=2 deferred-rescale cadence and add a MINIMAL distance-1 V prefetch ring in PV (mirror of the v63 K-ring): carry ONE extra V pair (v_v0_next/v_v1_next) across the PV loop so tile dt+1's V load retires in tile dt's WMMA shadow. Only ONE extra fragment pair live (vs v151's 4-load burst that lost, vs v150 CHUNK=4 that lost) -> removes the per-tile head-of-tile V-load bubble v140's pure interleave leaves exposed without inflating PV VGPR/waitcnt pressure. Second load (base+16) reuses base-addr VGPR (v149 trick). Identical V bytes + per-fragment rescale/WMMA order -> bit-exact, dVT-wired
template<class T> __global__ void opus_attn_gfx1201_kernel_v153(opus_attn_kargs); // v153: REVERT to v140 champion (93.91 TFLOPS, CHUNK=2) exactly, then round-35 reviewer NEXT -- a ZERO-LIVENESS PV priority split: drop to prio0 around ONLY the two V global_loads per D-tile (a load needs no matrix pipe, so issuing it at prio2 only steals the shared issue slot from co-resident waves' ready WMMAs with no benefit), then raise back to prio2 for the two PV WMMAs that consume them. NO extra V fragment carried live (v152 ring/v151 burst/v150 CHUNK=4 all LOST by adding V liveness). The V-load issue slot has never been re-prioritized (v136/v146 toggled the rescale VALU, not loads). Same V bytes + per-fragment WMMA order -> bit-exact, dVT-wired
...
static void run_opus_attn_gfx1201(int version, opus_attn_kargs k) {
    switch (version) {
        ...
        case 142: launch_<128, 32>(k, opus_attn_gfx1201_kernel_v142<opus_attn_traits<128, 32, 128>>); break;
        case 143: launch_<128, 32>(k, opus_attn_gfx1201_kernel_v143<opus_attn_traits<128, 32, 128>>); break;
        case 144: launch_<128, 32>(k, opus_attn_gfx1201_kernel_v144<opus_attn_traits<128, 32, 128>>); break;
        case 145: launch_<128, 32>(k, opus_attn_gfx1201_kernel_v145<opus_attn_traits<128, 32, 128>>); break;
        case 146: launch_<128, 32>(k, opus_attn_gfx1201_kernel_v146<opus_attn_traits<128, 32, 128>>); break;
        case 147: launch_<128, 32>(k, opus_attn_gfx1201_kernel_v147<opus_attn_traits<128, 32, 128>>); break;
        case 148: launch_<128, 32>(k, opus_attn_gfx1201_kernel_v148<opus_attn_traits<128, 32, 128>>); break;
        case 149: launch_<128, 32>(k, opus_attn_gfx1201_kernel_v149<opus_attn_traits<128, 32, 128>>); break;
        case 150: launch_<128, 32>(k, opus_attn_gfx1201_kernel_v150<opus_attn_traits<128, 32, 128>>); break;
        case 151: launch_<128, 32>(k, opus_attn_gfx1201_kernel_v151<opus_attn_traits<128, 32, 128>>); break;
        case 152: launch_<128, 32>(k, opus_attn_gfx1201_kernel_v152<opus_attn_traits<128, 32, 128>>); break;
        case 153: launch_<128, 32>(k, opus_attn_gfx1201_kernel_v153<opus_attn_traits<128, 32, 128>>); break;
