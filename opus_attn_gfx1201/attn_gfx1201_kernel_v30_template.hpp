// v30 — BLOCK_N=32 + fused rescale into PV loop (v26+v27 combo)
...
__device__ static inline bf16_t bf16_fast30(fp32_t f) {
    unsigned int x = __builtin_bit_cast(unsigned int, f);
    x += 0x7FFF + ((x >> 16) & 1);
    return static_cast<bf16_t>(x >> 16);
}

__device__ static inline fp32_t v30_cross_half_max(fp32_t v) {
    int x = __builtin_bit_cast(int, v);
    int other = __builtin_amdgcn_ds_bpermute((threadIdx.x ^ 16) << 2, x);
    return __builtin_fmaxf(v, __builtin_bit_cast(fp32_t, other));
}

__device__ static inline fp32_t v30_cross_half_sum(fp32_t v) {
    int x = __builtin_bit_cast(int, v);
    int other = __builtin_amdgcn_ds_bpermute((threadIdx.x ^ 16) << 2, x);
...
__global__ void opus_attn_gfx1201_kernel_v30(opus_attn_kargs k)
{
    ...
    for (int kt = 0; kt < DK; ++kt) {
        ...
        #pragma unroll
        for (int j = 0; j < 8; ++j)
            v_q[kt][j] = bf16_fast30(bf16_to_f32(v_q[kt][j]) * qscale);
    }
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        // QKT: two 16×16 sub-tiles for BLOCK_N=32
        ...
        row_max = v30_cross_half_max(row_max);
        ...
        row_sum = v30_cross_half_sum(row_sum);
        l_row += row_sum;

        bf16x8_t v_p0, v_p1;
        #pragma unroll
        for (int j = 0; j < 8; ++j) v_p0[j] = bf16_fast30(v_s0[j]);
        #pragma unroll
        for (int j = 0; j < 8; ++j) v_p1[j] = bf16_fast30(v_s1[j]);

        // PV with fused rescale and software-pipelined V loads
        bf16x8_t v_pf = *reinterpret_cast<const bf16x8_t*>(
            Vp + (n_base + col16) * stride_n + row8);

        #pragma unroll
        for (int dt = 0; dt < DK; ++dt) {
            // Fused rescale: rescale this D-tile's 8 accumulators
            if (need_rescale) {
                #pragma unroll
    ...
    for (int dt = 0; dt < DK; ++dt) {
        bf16x8_t o_pack;
        #pragma unroll
        for (int j = 0; j < 8; ++j) o_pack[j] = bf16_fast30(v_o[dt][j] * inv);
