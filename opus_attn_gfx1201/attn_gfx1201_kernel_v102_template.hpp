// v102 -- v100 (v63 K-prefetch + v43 batched pre-PV rescale) + minimal asymmetric
//         dt=0 V-load hoist to hide the FIRST PV load latency behind softmax.
//
// Motivation (round-3, reviewer-guided): in v100 the entire softmax (row-max reduce,
// rescale, 16x exp2, row-sum, 16x bf16 pack) completes BEFORE any PV V-load issues --
// every V load lives inside the PV loop. So dt=0's global V-load latency is fully
// EXPOSED at PV start: the matrix pipe stalls on the first WMMA waiting for its V
// operand. v101 tried full rolling V double-buffering and REGRESSED (85.66) because it
// injected a global load between every WMMA pair inside the very short PV window,
// wrecking WMMA issue density.
//
// v102 does the *minimal* asymmetric fix: hoist ONLY the dt=0 V loads up to right after
// the QKT WMMAs, BEFORE the softmax VALU. Those two loads then execute concurrently with
// the long softmax compute window, so PV's first WMMA finds its operand already resident.
// The PV loop body for dt>=1 stays byte-identical to v100 (tight, no per-iter prefetch
// churn) -- dt>=1 load latency is hidden behind the previous dt's WMMAs (matrix pipe
// already busy). Cost: only +2 bf16x8 (8 VGPR) of liveness across softmax, well under
// v101's full-buffer footprint. Numerically bit-identical to v100: same loads, same math,
// only load *timing* moves.
...
__global__ void opus_attn_gfx1201_kernel_v102(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        // ---- Asymmetric dt=0 V-load hoist (v102) ----
        // Issue the FIRST PV tile's V loads now so their global latency overlaps the
        // softmax VALU below. By the time PV starts, dt=0's operands are resident.
        bf16x8_t v_v0_first = *reinterpret_cast<const bf16x8_t*>(VTp + (col16) * vt_stride_d + n_base + row8);
        bf16x8_t v_v1_first = *reinterpret_cast<const bf16x8_t*>(VTp + (col16) * vt_stride_d + n_base + 16 + row8);
        ...
        // dt=0 consumes the hoisted operands; dt>=1 keeps v100's tight in-loop loads.
        #pragma unroll
        for (int dt = 0; dt < DK; ++dt) {
            bf16x8_t v_v0, v_v1;
            if (dt == 0) {
                ...
                v_v1 = *reinterpret_cast<const bf16x8_t*>(vt_addr1);
            }
