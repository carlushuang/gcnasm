// v150 -- v140 champion (93.91 TFLOPS) + ONE bit-exact, zero-VGPR structural
// change: sweep the PV rescale/load/WMMA CHUNK from 2 to 4. ROUND-33 (lane B).
//
// ================= WHY THIS, AND WHY IT IS GENUINELY NEW =====================
// The s_setprio scheduling space around the softmax island is now PROVABLY
// saturated: rounds 24-31 swept every prio1/prio0 boundary inside the softmax
// tail (v141..v146) and every PV address/cadence micro-tweak (v147 K-carry
// 88.46, v148 CHUNK=1 93.25, v149 vt_addr1=+16 93.25) -- none beat v140's CHUNK=2
// 93.91. The round-32 reviewer's own NEXT-FOCUS conceded the limiter is "PV WMMA
// issue-window / V-load-to-WMMA latency, not integer address arithmetic", and
// that "CHUNK=1 is confirmed bad here; it fragmented the PV cadence".
//
// That last sentence names the lever. The PV CHUNK is the granularity of the
// rescale->load->WMMA grouping, and within the EXACT winning v140 schedule the
// measured trend is MONOTONE in CHUNK:
//       CHUNK=1 (v148) = 93.25   <   CHUNK=2 (v140) = 93.91
// i.e. coarser chunking = longer uninterrupted V-load + WMMA bursts = better.
// CHUNK=4 is the next point on that line and has NEVER been run on the winning
// schedule (every chunked version v111..v149 used CHUNK in {1,2}; v135's
// monolithic placement put rescale at prio0 in the softmax island, a DIFFERENT
// structure, not a CHUNK sweep). This is the one untried structural granularity
// the reviewer's root-cause directly points at.
//
// MECHANISM (why CHUNK=4 should beat CHUNK=2 on V-load latency):
//   With CHUNK=4 the unrolled chunk body issues FOUR V-load pairs (8 b128
//   global_loads) and EIGHT PV WMMAs as one contiguous burst. The compiler's
//   in-burst scheduler can now hoist up to 8 in-flight V loads ahead of the WMMA
//   chain instead of v140's 4 -- DOUBLING the memory-level parallelism in the PV
//   region, which is exactly the V-load-to-WMMA latency the reviewer flagged as
//   the limiter. Coarser bursts also mean fewer rescale-VALU punctuation points
//   interrupting the prio-2 matrix issue stream (the same de-punctuation win that
//   took CHUNK=1->2), now taken one step further.
//
// REGISTER / OCCUPANCY SAFETY:
//   v140 measured 197 VGPR -> 7 waves/SIMD. CHUNK=4 raises peak V-fragment
//   liveness only if the scheduler chooses to fly all 8 V pairs at once; the
//   loads are still consumed in program order by their WMMAs, so worst-case
//   added liveness is a few transient V regs -- well inside the 7-wave budget,
//   nowhere near the BLOCK_N=64 spill cliff (133 VGPR over). If it did cross the
//   occupancy tier the harness would simply report a lower number; the change
//   carries no correctness risk.
//
// ================= WHY IT IS BIT-EXACT ====================================
// CHUNK only regroups WHICH D-tiles' rescale + load + WMMA are emitted together;
// it changes NO arithmetic. Each v_o[dt] is rescaled by the identical `rescale`
// scalar and accumulated by the identical pair of WMMAs in the identical order
// (dt ascending, v0 then v1). No fp32 add/mul is reassociated, no softmax
// reduction order moves, no bf16 pack changes. Every per-fragment op sequence is
// byte-identical to v140 -> max_abs stays 0.0001, n_bad=0.
//
// (Original v140 rationale retained below for context.)
...
__device__ static inline bf16_t bf16_fast150(fp32_t f) {
    unsigned int x = __builtin_bit_cast(unsigned int, f);
    x += 0x7FFF + ((x >> 16) & 1);
    return static_cast<bf16_t>(x >> 16);
}

__device__ static inline fp32_t v150_cross_half_max(fp32_t v) {
    int x = __builtin_bit_cast(int, v);
    int other = __builtin_amdgcn_ds_bpermute((threadIdx.x ^ 16) << 2, x);
    return __builtin_fmaxf(v, __builtin_bit_cast(fp32_t, other));
}

__device__ static inline fp32_t v150_cross_half_sum(fp32_t v) {
    int x = __builtin_bit_cast(int, v);
    int other = __builtin_amdgcn_ds_bpermute((threadIdx.x ^ 16) << 2, x);
...
__global__ void opus_attn_gfx1201_kernel_v150(opus_attn_kargs k)
{
    ...
    for (int kt = 0; kt < DK; ++kt) {
        ...
        #pragma unroll
        for (int j = 0; j < 8; ++j)
            v_q[kt][j] = bf16_fast150(bf16_to_f32(v_q[kt][j]) * qscale);
    }
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        row_max = v150_cross_half_max(row_max);
        ...
        row_sum = v150_cross_half_sum(row_sum);
        l_row += row_sum;

        bf16x8_t v_p0, v_p1;
        #pragma unroll
        for (int j = 0; j < 8; ++j) v_p0[j] = bf16_fast150(v_s0[j]);
        #pragma unroll
        for (int j = 0; j < 8; ++j) v_p1[j] = bf16_fast150(v_s1[j]);
    ...
    for (int dt = 0; dt < DK; ++dt) {
        bf16x8_t o_pack;
        #pragma unroll
        for (int j = 0; j < 8; ++j) o_pack[j] = bf16_fast150(v_o[dt][j] * inv);
