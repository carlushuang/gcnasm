// v120 -- BYTE-FOR-BYTE v100 kernel body + ONE non-arithmetic occupancy directive.
// Round 10 (v119) and the whole v111-v119 family contaminated the arithmetic
// (max_abs=0.0360 instead of v100's 0.0001) and/or regressed primary TFLOPS by
// perturbing the QKT K-load schedule. The reviewer mandate: restore an exact v100
// body (re-establish max_abs=0.0001) and apply ONLY a non-arithmetic, order-
// preserving live-range trim OUTSIDE the QKT K schedule.
//
// CHANGE vs v100: the SOLE difference is __launch_bounds__(BLOCK_SIZE, 1) -> (.., 2).
// The kernel body is otherwise identical to v100 (verified by diff). The second
// __launch_bounds__ arg is a minimum-workgroups-per-CU hint: it directs the
// register allocator to fit 2 WGs/CU, lowering the VGPR target below v100's ~205
// (the stated 7-wave occupancy wall) toward ~192 -> 8 waves/SIMD (playbook lever
// #4 / mining note: 190 VGPR -> 8 waves). Because it touches NO instruction,
// memory order, WMMA schedule, or fp32 accumulation, it is bit-exact by
// construction (unlike v56's brutal amdgpu_num_vgpr). The bet: one extra resident
// wave of latency hiding for free, IF the allocator hits the target without
// spilling. If it spills, perf is flat-to-slightly-down but still bit-exact.
//
// Underlying body retains BOTH original wins:
//   * v63: software-pipelined next-D-tile K load hides global K-load latency.
//   * v43: batched pre-PV rescale of all 8 O accumulators -> branch-free PV loop.
