// v126 -- v111's fast chunked-rescale PV schedule, REGISTERED CORRECTLY in the
// host V-pointer (dVT) dispatch. This is the ROUND-19 ROOT-CAUSE FIX.
//
...
//
// v126 is the v111 schedule (identical device code) but is added to BOTH the
...
// fp32x8 O accumulators (64 fp32/lane) in one VALU region. v126 splits the
...
__global__ void opus_attn_gfx1201_kernel_v126(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        if (need_rescale) {
            ...
            // one VALU block). v126 defers it into 2-tile chunks interleaved with
