// v100 -- v63 (K double-buffer prefetch in QKT) + v43 (batched pre-PV rescale).
// These two wins are orthogonal and were never combined:
//   * v63 hides global K-load latency behind the QKT WMMA issue window by
//     software-pipelining the next D-tile's K load (90.6 -> 91-93).
//   * v43 hoists the online-softmax rescale of all 8 O accumulators OUT of the
//     per-D-tile PV loop into one branch-guarded pass BEFORE PV, so V loads issue
//     concurrently with the rescale FMAs and the PV loop runs branch-free (88.7 -> 90.6).
// v63 regressed the rescale back into the PV loop; v100 keeps BOTH optimizations.
