// v101 -- v100 (v63 K double-buffer QKT + v43 batched pre-PV rescale)
//          + V software-pipelining in the PV phase.
//
// Round-1 review proposed "chunked pre-PV rescale" on the theory that v100 had
// crossed a VGPR/occupancy cliff. Static resource analysis on this box REFUTES
// that: v100 compiles to 159 VGPR, occupancy 9 waves/SIMD, ZERO spill -- it is
// already tight. Chunking the rescale (interleaving it back into PV) actually
// *raised* pressure to 197 VGPR / occupancy 7 (extra live V buffers), the
// opposite of the goal. So v100's 88.07 is not a register-pressure problem.
//
// The remaining exposed latency is the PV-phase V global loads: v100 (like v63)
// software-pipelines the K loads in QKT but loads V *synchronously* right before
// each PV WMMA, so V-load latency is on the critical path. v101 applies the
// same double-buffer trick to V (playbook lever #3, "V SW-pipelining"): preload
// D-tile 0's V, then prefetch D-tile dt+1's V while dt's PV WMMA executes,
// hiding the V-load latency behind the matrix-pipe issue window.
//
// Crucially this stays at 159 VGPR / occupancy 9 / 0 spill (verified with
// -Rpass-analysis=kernel-resource-usage) -- the next-V buffer reuses the slots
// the old synchronous V load already needed, so there is no occupancy cost.
// Bit-exact vs v100: identical math, only the V-load *timing* changes.
...
__global__ void opus_attn_gfx1201_kernel_v101(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        // ---- PV: branch-free (rescale already applied above) + V SW-pipeline ----
        // Preload D-tile 0's V, then prefetch the next D-tile's V while the
        // current PV WMMA runs, hiding V global-load latency behind the matrix
        // pipe -- the PV-side mirror of the QKT K prefetch above.
        bf16x8_t v_v0_next = *reinterpret_cast<const bf16x8_t*>(VTp + (col16) * vt_stride_d + n_base + row8);
        bf16x8_t v_v1_next = *reinterpret_cast<const bf16x8_t*>(VTp + (col16) * vt_stride_d + n_base + 16 + row8);
        #pragma unroll
        for (int dt = 0; dt < DK; ++dt) {
