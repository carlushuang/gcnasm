// v131 -- v126's fast chunked-rescale PV schedule + ASYMMETRIC WAVE-PRIORITY
// SCHEDULING (s_setprio) across the QK^T and PV WMMA regions. ROUND-14 (lane B).
//
// ================= HYPOTHESIS ==============================================
// v126 (the current champion, 91.24 primary, bit-exact) is matrix-pipe ISSUE
// bound: 7-8 wave32s share one CU's single WMMA pipe, so when several resident
// waves each have a WMMA ready in the same cycle the hardware round-robins the
// issue slot. Which wave wins matters because the two matrix regions are NOT
// symmetric in their ability to self-fill the pipe:
//   * QK^T has only TWO independent WMMA chains (v_s0, v_s1) of depth DK=8, and
//     each step is gated on a global K load (the v63 K-prefetch ring). LOW ILP,
//     bubble-prone -- when a QK^T wave finally has a WMMA ready it should WIN the
//     slot, because it cannot keep the pipe busy from its own ILP.
//   * PV has EIGHT independent accumulator chains (v_o[0..7]) of depth 2. HIGH
//     ILP -- it self-fills the matrix pipe and can afford to lose an occasional
//     issue slot to a bubble-prone QK^T wave on another wave.
// So we raise priority to 2 around the QK^T WMMA burst and use 1 around the PV
// WMMA burst; the pure-VALU softmax island between them (cross-lane max/sum,
// exp2, bf16 pack) runs at priority 0 so it never out-competes another wave's
// ready WMMA. This is the lever lane A independently proved bit-exact and worth
// +1-2 TFLOPS (their v135 92.42 / v138 93.38); it has never been tried in lane B.
//
// ================= WHY IT IS BIT-EXACT ====================================
...
// is byte-identical to v126. -> max_abs stays 0.0001 (same as v126). The proven
// rule from lane A ("never lower priority adjacent to a WMMA burst") is honored:
// the only prio-0 region is the pure-VALU softmax island, with no WMMA in it.
//
// ================= ROOT-CAUSE / WIRING NOTE (inherited from v126) ==========
// v131 MUST be in the host dVT (pre-transposed V, layout [B,H,D,N]) allow-list;
...
// kernel was always correct; only the wiring was wrong. Host registers v131 in
...
__global__ void opus_attn_gfx1201_kernel_v131(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        // PV is high-ILP (8 independent v_o chains) -> medium priority 1. It
        // self-fills the matrix pipe, so it can yield the occasional issue slot
        // to a bubble-prone QK^T wave (prio 2) on another resident wave without
        // losing throughput. Priority is raised ONCE here and held across the
        // whole PV region (chunked rescale + WMMAs) -- no toggle inside the WMMA
        // burst (lane A's v136 proved per-chunk toggling regresses).
