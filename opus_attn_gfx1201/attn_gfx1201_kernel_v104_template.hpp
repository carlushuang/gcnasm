// v104 -- v100 (v63 K-prefetch + v43 batched pre-PV rescale) + dt=0 PV PROLOGUE.
// Background: v102 hoisted dt=0's V across the whole softmax window (VGPR tax, 86.6);
// v103 hoisted only ONE dt=0 fragment before the bf16 pack (87.1) -- but the dt=0
// v_v1 global load still sat in front of the first PV WMMA, so the head-of-chain was
// still memory-gated. v104 instead PEELS dt=0 out of the PV loop into an explicit
// prologue: right after P is packed, load BOTH dt=0 V operands (v0,v1), issue the two
// dt=0 WMMAs immediately, then enter a clean steady-state loop for dt=1..DK-1 with NO
// per-iteration `if`. This removes the dt==0 branch/select residue v103 left in the
// unrolled loop, gives the compiler a single unambiguous schedule (pack -> dt=0 loads ->
// dt=0 WMMAs -> steady state), and lets the dt>=1 loads overlap the dt=0 WMMA latency
// (using WMMA as the hiding window instead of adding live V operands across softmax).
...
__global__ void opus_attn_gfx1201_kernel_v104(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        // ---- PV dt=0 PROLOGUE (v104) ----
        // Load BOTH dt=0 V operands immediately after P is packed, fire the two dt=0
        // WMMAs at once so the head-of-chain is no longer gated by an interleaved load,
        // then fall into a branch-free steady-state loop. The dt>=1 loads below issue
        // while these dt=0 WMMAs are still in flight (WMMA latency = the hiding window).
        {
            const bf16_t* vt0_a0 = VTp + col16 * vt_stride_d + n_base + row8;
            const bf16_t* vt0_a1 = VTp + col16 * vt_stride_d + n_base + 16 + row8;
            bf16x8_t v_v0 = *reinterpret_cast<const bf16x8_t*>(vt0_a0);
            bf16x8_t v_v1 = *reinterpret_cast<const bf16x8_t*>(vt0_a1);
            v_o[0] = __builtin_amdgcn_wmma_f32_16x16x16_bf16_w32_gfx12(v_v0, v_p0, v_o[0]);
            v_o[0] = __builtin_amdgcn_wmma_f32_16x16x16_bf16_w32_gfx12(v_v1, v_p1, v_o[0]);
        }

        // ---- PV steady state: dt=1..DK-1, branch-free, no dt==0 select residue ----
        #pragma unroll
        for (int dt = 1; dt < DK; ++dt) {
