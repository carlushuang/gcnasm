...
// opus_attn_gfx1201 v17 — v13 with BLOCK_N=64 (4 sub-tiles)
//
// Over v13 (BLOCK_N=32):
//   - BLOCK_N=64: processes 4 N-subtiles per outer iteration
//   - Doubles the compute per loop iter, better amortizing loop overhead
//   - Uses original bf16_from_f32 (provides latency hiding)
//
...
__device__ static inline fp32_t v17_fmaxf(fp32_t a, fp32_t b) { return a > b ? a : b; }

__device__ static inline fp32_t v17_cross_half_max(fp32_t v) {
    int x = __builtin_bit_cast(int, v);
    int other = __builtin_amdgcn_ds_bpermute((threadIdx.x ^ 16) << 2, x);
    return v17_fmaxf(v, __builtin_bit_cast(fp32_t, other));
}

__device__ static inline fp32_t v17_cross_half_sum(fp32_t v) {
    int x = __builtin_bit_cast(int, v);
    int other = __builtin_amdgcn_ds_bpermute((threadIdx.x ^ 16) << 2, x);
...
__global__ void opus_attn_gfx1201_kernel_v17(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        // mma0 SWAP for all N_SUB=4 sub-tiles
        fp32x8_t v_s[N_SUB];
        #pragma unroll
        ...
        // Online softmax over N_SUB*8 = 32 values
        fp32_t row_max = v_s[0][0];
        #pragma unroll
        for (int ns = 0; ns < N_SUB; ++ns) {
            #pragma unroll
            for (int j = (ns == 0 ? 1 : 0); j < 8; ++j)
                row_max = v17_fmaxf(row_max, v_s[ns][j]);
        }
        row_max = v17_cross_half_max(row_max);

        const fp32_t new_m = v17_fmaxf(m_row, row_max);
        ...
        row_sum = v17_cross_half_sum(row_sum);
        l_row = l_row * rescale + row_sum;

        // mma1 SWAP for all N_SUB sub-tiles
        #pragma unroll
        for (int ns = 0; ns < N_SUB; ++ns) {
            bf16x8_t v_p;
            #pragma unroll
