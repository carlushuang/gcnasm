// v105 -- v104 (dt=0 PV prologue) + INTERLEAVED dt=0 prologue (load-WMMA-load-WMMA).
// Background: v104 peeled dt=0 into a prologue but loaded BOTH V operands (v0,v1) before
// issuing either WMMA, so the head-of-chain WMMA0 was still gated behind the v_v1 VMEM
// latency (both loads must retire before the first WMMA can issue under a single waitcnt).
// v105 reorders the prologue to: load v_v0 -> issue WMMA0 -> load v_v1 -> issue WMMA1.
// Now WMMA0 can fire as soon as ONLY v_v0 has retired, and the v_v1 global load overlaps
// WMMA0's compute latency (WMMA = the hiding window) instead of being waited on up-front.
// WMMA1 still serially accumulates into v_o[0] after WMMA0, so accumulation order -- and
// thus bf16 bit-exactness -- is identical to v104. This is the reviewer's NEXT FOCUS:
// ungate the dt=0 chain head by splitting the two prologue loads around the first WMMA.
...
__global__ void opus_attn_gfx1201_kernel_v105(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        // ---- PV dt=0 PROLOGUE, interleaved (v105) ----
        // load v_v0 -> WMMA0 -> load v_v1 -> WMMA1. WMMA0 fires once only v_v0 has
        // retired; the v_v1 load then overlaps WMMA0's latency instead of being waited
        // on before either WMMA. Serial accumulation into v_o[0] is unchanged (bit-exact).
        {
