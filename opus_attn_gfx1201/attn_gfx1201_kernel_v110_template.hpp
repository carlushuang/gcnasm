// v110 -- v100 (v63 K-prefetch + v43 batched pre-PV rescale) + split two-pass PV.
//
// v100's PV inner loop issues, per D-tile dt, TWO WMMAs that both accumulate into
// the SAME register v_o[dt]:
//     v_o[dt] = wmma(V0, p0, v_o[dt]);   // writes v_o[dt]
//     v_o[dt] = wmma(V1, p1, v_o[dt]);   // RAW-depends on the line above
// The second WMMA cannot issue until the first retires -> the matrix pipe stalls
// on a read-after-write hazard every 2 WMMAs, so PV runs at ~half matrix-pipe ILP.
//
// v110 splits PV into two passes over dt:
//     pass A:  for dt: v_o[dt] = wmma(V0, p0, v_o[dt]);   // 8 INDEPENDENT writes
//     pass B:  for dt: v_o[dt] = wmma(V1, p1, v_o[dt]);   // 8 INDEPENDENT writes
// Within each pass consecutive WMMAs write DIFFERENT accumulators (v_o[0..7]), so
// they are mutually independent and issue back-to-back. Pass B's v_o[dt] depends on
// pass A's v_o[dt], but they are DK(=8) WMMAs apart -> the hazard is fully hidden.
// Per-accumulator order is still V0*p0 THEN V1*p1, so the result is BIT-EXACT to v100.
//
// Coupled critical-path win: only exp2(s0)+pack(p0) are needed before pass A starts,
// so PV begins after ~half the softmax tail. exp2(s1)+pack(p1)+the row_sum tree
// reduction (incl. its ds_bpermute LDS latency) are deferred to between the two
// passes, where they overlap pass-A WMMAs (transcendental/LDS hide in the matrix
// shadow -- lever #6, realized structurally rather than via fragile scheduling hints).
// v110 also holds only ONE V fragment live at a time (vs v100's v_v0+v_v1), trimming
// PV V-load register liveness.
...
__global__ void opus_attn_gfx1201_kernel_v110(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        // ---- row max over both score subtiles ----
        ...
        if (need_rescale) {
            ...
            l_row *= rescale;
            // ---- Batched pre-PV rescale (v43): all O accumulators in one pass ----
            #pragma unroll
        ...
        // ---- p0 path: only what pass A needs, on the QKT->PV critical path ----
        #pragma unroll
        ...
        // ---- PV pass A: V0 * p0 into every accumulator (8 independent WMMAs) ----
        #pragma unroll
        for (int dt = 0; dt < DK; ++dt) {
        ...
        // ---- deferred s1 path + sum: overlaps pass-A matrix pipe (lever #6) ----
        #pragma unroll
        ...
        // ---- PV pass B: V1 * p1 into every accumulator (8 independent WMMAs) ----
        #pragma unroll
        for (int dt = 0; dt < DK; ++dt) {
