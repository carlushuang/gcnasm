// v121 -- v100 (v63 K double-buffer in QKT + v43 batched pre-PV rescale) with a
// SYMMETRIC V software-pipeline in the PV phase (playbook lever 3 / v78).
//
// v100 already software-pipelines K loads in QKT (preload D-tile 0, prefetch the
// next D-tile's K while the current WMMA runs) -- hiding global K-load latency in
// the WMMA issue window. BUT the PV loop still loads V SYNCHRONOUSLY: each
// iteration loads v_v0/v_v1 and immediately consumes them in the next WMMA, so the
// head-of-iteration VMEM latency is exposed on the critical path of every D-tile.
//
// v121 applies the identical, proven-bit-exact prefetch pattern from QKT to PV:
// preload D-tile 0's V operands before the loop, then prefetch the NEXT D-tile's V
// while the current D-tile's two PV WMMAs execute. dt+1's V load gets the full WMMA
// issue shadow of dt to land before it is consumed.
//
// CRITICAL CORRECTNESS NOTE: this is ARITHMETICALLY BYTE-IDENTICAL to v100. Same V
// operands, same v_o[dt] accumulation order, same FP ops -- only the *issue time*
// of the V global loads moves earlier. It does NOT touch the QKT K schedule (source
// of the v111-v120 max_abs=0.0360 contamination) and does NOT reassociate any
// softmax/rescale arithmetic (v100's monolithic pre-PV rescale is kept verbatim).
// Expect max_abs ~= 0.0001 (v100-identical), n_bad == 0.
...
__global__ void opus_attn_gfx1201_kernel_v121(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        //      software-pipelined V loads (v121, symmetric to the QKT K prefetch).
        // Preload D-tile 0's V, then prefetch dt+1's V while dt's WMMAs execute so
        // the global V-load latency hides in the WMMA issue shadow. Arithmetically
        // identical to v100: same operands, same v_o[dt] accumulation order. ----
        bf16x8_t v_v0_next = *reinterpret_cast<const bf16x8_t*>(
            VTp + (0 * W_K + col16) * vt_stride_d + n_base + row8);
        bf16x8_t v_v1_next = *reinterpret_cast<const bf16x8_t*>(
            VTp + (0 * W_K + col16) * vt_stride_d + n_base + 16 + row8);
        ...
        for (int dt = 0; dt < DK; ++dt) {
            ...
            if (dt + 1 < DK) {
                v_v0_next = *reinterpret_cast<const bf16x8_t*>(
                    VTp + ((dt+1) * W_K + col16) * vt_stride_d + n_base + row8);
                v_v1_next = *reinterpret_cast<const bf16x8_t*>(
                    VTp + ((dt+1) * W_K + col16) * vt_stride_d + n_base + 16 + row8);
            }
