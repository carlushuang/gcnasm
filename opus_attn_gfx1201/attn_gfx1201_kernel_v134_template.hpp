...
__global__ void opus_attn_gfx1201_kernel_v134(opus_attn_kargs k)
{
    ...
    for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
        ...
        // ---- v134: hoist the PV prio-2 raise ABOVE the bf16 P-pack ----------
        // The bf16 P-pack below produces v_p0/v_p1, which ARE the B-operand of
        // the very FIRST PV WMMA. In v132 the prio-2 raise sat AFTER this pack
        // (at the PV-loop head), so the pack itself executed at the softmax
        // island's prio 0. Under 7-8 resident wave32s saturating the single
        // matrix/VALU issue port, a prio-0 pack systematically loses its issue
        // slots to other waves' prio-2 WMMAs -> the first PV WMMA cannot become
        // *ready* until the demoted pack drains, exposing a head-of-PV bubble
        // that priority on the WMMAs alone cannot hide. This is EXACTLY the bug
        // v132 fixed for QK^T (it hoisted the raise above the K preloads so the
        // first QK^T WMMA arbitrated at prio 2); v134 applies the identical fix
        // to the PV-feeding tail. The genuinely-yieldable softmax work
        // (cross-lane max/sum, exp2, row-sum fold) stays at prio 0 ABOVE this
        // point so it still hides in other waves' WMMA shadows (playbook lever
        // 6) -- only the short bf16 pack that gates PV is promoted.
        //
        // ZERO register cost: s_setprio is a single SALU instruction that moves
        // no data and reassociates no fp32 add. Every v_p[j]=bf16_fast100(...),
        // every v_o multiply/WMMA operand order is byte-identical to v132.
        // -> max_abs stays 0.0001 (bit-exact). This is the only lever left that
        // does NOT add live registers (V/K prefetch v133=71.4 and persistent-WG
        // v129/v130=54-84 all crossed the occupancy cliff); it stays at v132's
        // 205-VGPR / 7-wave footprint exactly.
        __builtin_amdgcn_s_setprio(2);
        bf16x8_t v_p0, v_p1;
        #pragma unroll
        ...
        // Uniform prio 2 on all WMMAs lets HW round-robin fairly. v134 already
        // raised prio to 2 above the bf16 P-pack (which gates the first PV WMMA),
        // so the priority is ALREADY 2 here -- no second s_setprio is emitted; the
        // whole PV region (pack + all WMMAs) runs at prio 2 with no toggle inside
        // the WMMA burst (lane A's v136 proved per-chunk toggling regresses).
