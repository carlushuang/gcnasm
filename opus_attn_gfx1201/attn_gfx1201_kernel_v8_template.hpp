...
// opus_attn_gfx1201 v8 — v6 (contig V load) with BLOCK_N=32 (2 sub-tiles per
// outer iter, online softmax). Goal: halve per-outer-iter softmax overhead.
// v5 tried BLOCK_N=32 with the original strided V load and only tied v0; with
// v6's contiguous-V optimization the softmax savings may now show up.
//
...
__device__ static inline fp32_t v8_fmaxf(fp32_t a, fp32_t b) { return a > b ? a : b; }

template<class T>
__launch_bounds__(T::BLOCK_SIZE, 1)
__global__ void opus_attn_gfx1201_kernel_v8(opus_attn_kargs k)
{
    ...
    for (int outer = 0; outer < num_outer_tiles; ++outer) {
        ...
        for (int nt = 0; nt < NT; ++nt) {
            ...
            for (int j = 0; j < 8; ++j) {
                fp32_t v = v_s[j];
                v = v8_fmaxf(v, __builtin_bit_cast(fp32_t, __builtin_amdgcn_ds_swizzle(__builtin_bit_cast(int, v), 0x041F)));
                v = v8_fmaxf(v, __builtin_bit_cast(fp32_t, __builtin_amdgcn_ds_swizzle(__builtin_bit_cast(int, v), 0x081F)));
                v = v8_fmaxf(v, __builtin_bit_cast(fp32_t, __builtin_amdgcn_ds_swizzle(__builtin_bit_cast(int, v), 0x101F)));
                v = v8_fmaxf(v, __builtin_bit_cast(fp32_t, __builtin_amdgcn_ds_swizzle(__builtin_bit_cast(int, v), 0x201F)));
                const fp32_t new_m = v8_fmaxf(m_row[j], v);
