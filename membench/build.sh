...
while getopts ":b:g:u:n:i:k:s:a:v:h" opt; do
  case $opt in
    b) block_size="$OPTARG" 
       any_option_set=1
       ;;
...
if [ $enable_config_json -eq 1 ]; then
  ...
  i=1
  PIXELS_MB=$(awk -v block="$block_size" -v grid="$grid_size" -v unroll="$unroll" -v inner="$inner" -v i="$i" 'BEGIN {print (((block * grid * 16 / 4 / 1024 / 1024) * unroll * inner) * i) }')
