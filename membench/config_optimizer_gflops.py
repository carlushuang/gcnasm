...
def main():
    ...
    with open(input_file, "r") as csvfile:
        ...
        for row in reader:
            ...
            cmd = "ARCH=" + arch + " bash build.sh"
            os.system(cmd)
            output = subprocess.check_output("./membench").decode("utf-8")
            print(output)
            ...
            if not os.path.exists("all_output.csv"):
                with open("all_output.csv", "w") as outfile:
                    outfile.write("BS,GS,UNROLL,PIXELS_MB,INNER,GFlops\n")

            with open("all_output.csv", "a") as outfile:
                outfile.write(f"{BS},{GS},{UNROLL},{PIXELS_MB},{INNER},{result_float}\n")
