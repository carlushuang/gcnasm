...
struct hw_reg_d {
    ...
    uint32_t se_id  ;
    uint32_t wave_id;
    uint32_t xcc_id ; // host not valid
};
...
int main(int argc, char ** argv)
{
    ...
    uint32_t cu_mask[] = {0x1, 0, 0};
    HIP_CALL(hipExtStreamCreateWithCUMask(&stream, sizeof(cu_mask) / sizeof(cu_mask[0]), cu_mask));

    {
        uint32_t cu_mask_[64];  // TODO: seems need be 64?
        HIP_CALL(hipExtStreamGetCUMask(stream, 64, &cu_mask_[0]));
        printf("cu_mask:%x, %x\n", cu_mask_[0], cu_mask_[1]);
    }
#endif
    int num_cu = [](){
    ...
    dump_hwreg<<<grids, BLOCK_SIZE, 0, stream>>>(dev_i, dev_o, grids);
    ...
    for(auto i = 0; i < grids*BLOCK_SIZE; i++) {
        ...
        d.se_id = o[5];
        d.wave_id = o[6];
        d.xcc_id = o[7]; // host not valid
        hw_v.push_back(d);
    }

    std::sort(hw_v.begin(), hw_v.end(), [&](auto x, auto y){
        if(x.xcc_id != y.xcc_id) return x.xcc_id < y.xcc_id;
        else if(x.se_id != y.se_id) return x.se_id < y.se_id;
        else return x.cu_id < y.cu_id; });

    for(auto v : hw_v) {
        printf("block:%4d, cu_id:%2u, sh_id:%2u, se_id:%2u, wave_id:%2u", v.blk_id, v.cu_id, v.sh_id, v.se_id, v.wave_id);
        printf(", xcc_id:%2u", v.xcc_id);
        printf("\n");
    }
}
