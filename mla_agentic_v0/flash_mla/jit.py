"""
JIT compilation module for flash_mla.

Generates Ninja build files, compiles on first use,
...
_PACKAGE_ROOT = Path(__file__).resolve().parent.parent  # mla/
CSRC_DIR = _PACKAGE_ROOT / "csrc"

CACHE_DIR = Path(
    os.getenv("FLASH_MLA_CACHE_DIR", Path.home() / ".cache" / "flash_mla")
)
JIT_DIR = CACHE_DIR / "jit"

ROCM_PATH = Path(os.getenv("ROCM_PATH", "/opt/rocm"))
AITER_INCLUDE_DIR = Path(
    os.getenv("AITER_INCLUDE_DIR", str(_PACKAGE_ROOT / "third_party" / "aiter" / "csrc" / "include"))
)
...
def _generate_ninja_build(
    ...
    """Generate a build.ninja for compiling HIP + C++ sources."""

    hipcc = str(ROCM_PATH / "bin" / "hipcc")
    cxx = os.environ.get("CXX", "g++")

    # tvm_ffi include/lib from pip
...
class JitSpec:
    ...
    def build_and_load(self) -> tvm_ffi.Module:
        verbose = os.environ.get("FLASH_MLA_JIT_VERBOSE", "0") == "1"
...
@functools.cache
def gen_flash_mla_module() -> JitSpec:
    """Create the JitSpec for the flash_mla_decode kernel."""
    return JitSpec(
        name="flash_mla",
        sources=[
            CSRC_DIR / "flash_mla_kernel.hip",
            CSRC_DIR / "tvm_api.cc",
        ],
    )
