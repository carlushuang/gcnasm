...
.set v_m_idx,           113
.set v_n_idx,           114
.set v_smem_store,      115
.set v_os_a,            116
.set v_os_b,            118
.set v_os_c,            v_lane_id
.set v_smem_load_a,     120
.set v_smem_load_b,     121
.set v_smem_store_c,    122
.set v_smem_load_c,     123
.set v_tmp,             124
.set v_end,             v_tmp+3
...
sgemm_128x128:
    ...
    v_lshrrev_b32 v[v_wave_id], 6, v0
    v_lshrrev_b32 v[v_wave_p], 1, v[v_wave_id]
    v_and_b32 v[v_wave_q], 1, v[v_wave_id]
    ...
    .v_u32_div_ss v_n_idx, s_bx, s_m_blocks, v_tmp, s_tmp
    v_mul_lo_u32 v[v_tmp], s[s_m_blocks], v[v_n_idx]
    v_sub_u32 v[v_m_idx], s[s_bx], v[v_tmp]
    s_lshl_b32 s[s_bs_a], s[s_lda], 3
    s_lshl_b32 s[s_bs_b], s[s_ldb], 3

    v_mov_b32 v[v_os_a], s[s_ptr_a]
    v_mov_b32 v[v_os_a+1], s[s_ptr_a+1]
    v_mov_b32 v[v_os_b], s[s_ptr_b]
    v_mov_b32 v[v_os_b+1], s[s_ptr_b+1]
    v_lshlrev_b32 v[v_tmp+3], 4, v[v_lane_lo]
    v_lshl_add_u32 v[v_tmp], v[v_m_idx], 9, v[v_tmp+3]
    v_lshl_add_u32 v[v_tmp+1], v[v_n_idx], 9, v[v_tmp+3]
    v_add_co_u32 v[v_os_a], vcc, v[v_tmp], v[v_os_a]
    v_addc_co_u32  v[v_os_a+1], vcc, 0, v[v_os_a+1], vcc
    v_add_co_u32 v[v_os_b], vcc, v[v_tmp+1], v[v_os_b]
    v_addc_co_u32  v[v_os_b+1], vcc, 0, v[v_os_b+1], vcc

    v_lshl_or_b32 v[v_tmp+3], v[v_wave_id], 1, v[v_lane_hi]
    v_mul_lo_u32 v[v_tmp], s[s_lda], v[v_tmp+3]
    v_mul_hi_u32 v[v_tmp+1], s[s_lda], v[v_tmp+3]
    v_mul_lo_u32 v[v_tmp+2], s[s_ldb], v[v_tmp+3]
    v_mul_hi_u32 v[v_tmp+3], s[s_ldb], v[v_tmp+3]

    v_add_co_u32 v[v_os_a], vcc, v[v_tmp], v[v_os_a]
    v_addc_co_u32  v[v_os_a+1], vcc, v[v_tmp+1], v[v_os_a+1], vcc
    v_add_co_u32 v[v_os_b], vcc, v[v_tmp+2], v[v_os_b]
    v_addc_co_u32  v[v_os_b+1], vcc, v[v_tmp+3], v[v_os_b+1], vcc

    global_load_dwordx4 v[v_p0:v_p0+3], v[v_os_a:v_os_a+1], off
    global_load_dwordx4 v[v_q0:v_q0+3], v[v_os_b:v_os_b+1], off
    ...
    v_lshl_or_b32 v[v_smem_load_c], v[v_wave_id], 10, v[v_tmp+1]

    v_lshlrev_b32 v[v_tmp], 4, v[v_lane_lo]
    v_lshl_add_u32 v[v_tmp+3], v[v_m_idx], 9, v[v_tmp]
    v_lshlrev_b32 v[v_tmp], 7, v[v_n_idx]
    v_lshl_or_b32 v[v_tmp+1], v[v_wave_id], 2, v[v_lane_hi]
    v_add_u32 v[v_tmp+1], v[v_tmp], v[v_tmp+1]
    v_mul_lo_u32 v[v_tmp], s[s_ldc], v[v_tmp+1]
    v_mul_hi_u32 v[v_tmp+1], s[s_ldc], v[v_tmp+1]
    v_mov_b32 v[v_os_c], s[s_ptr_c]
    v_mov_b32 v[v_os_c+1], s[s_ptr_c+1]
    v_add_co_u32 v[v_os_c], vcc, v[v_tmp], v[v_os_c]
    v_addc_co_u32  v[v_os_c+1], vcc, v[v_tmp+1], v[v_os_c+1], vcc
    v_add_co_u32 v[v_os_c], vcc, v[v_tmp+3], v[v_os_c]
    v_addc_co_u32  v[v_os_c+1], vcc, 0, v[v_os_c+1], vcc
...
L_sgemm128x128_k_loop_start:
    ...
    v_add_co_u32 v[v_os_a], vcc, s[s_bs_a], v[v_os_a]
    v_addc_co_u32  v[v_os_a+1], vcc, 0, v[v_os_a+1], vcc
    v_add_co_u32 v[v_os_b], vcc, s[s_bs_b], v[v_os_b]
    v_addc_co_u32  v[v_os_b+1], vcc, 0, v[v_os_b+1], vcc
...
L_sgemm128x128_k_loop_end:
    ...
    .rept 4
        ...
        s_waitcnt lgkmcnt(3)
        global_store_dwordx4 v[v_tmp:v_tmp+1], v[v_c+4*.cid+0 :v_c+4*.cid+3 ] off
        s_mul_i32 s[s_tmp], .cof+16, s[s_ldc]
        v_add_co_u32 v[v_tmp], vcc, s[s_tmp], v[v_os_c]
        v_addc_co_u32  v[v_tmp+1], vcc, 0, v[v_os_c+1], vcc
        s_waitcnt lgkmcnt(2)
        global_store_dwordx4 v[v_tmp:v_tmp+1], v[v_c+4*.cid+16:v_c+4*.cid+19] off
        s_mul_i32 s[s_tmp], .cof+64, s[s_ldc]
        v_add_co_u32 v[v_tmp], vcc, s[s_tmp], v[v_os_c]
        v_addc_co_u32  v[v_tmp+1], vcc, 0, v[v_os_c+1], vcc
        s_waitcnt lgkmcnt(1)
        global_store_dwordx4 v[v_tmp:v_tmp+1], v[v_c+4*.cid+4 :v_c+4*.cid+7 ] off
        s_mul_i32 s[s_tmp], .cof+80, s[s_ldc]
        v_add_co_u32 v[v_tmp], vcc, s[s_tmp], v[v_os_c]
        v_addc_co_u32  v[v_tmp+1], vcc, 0, v[v_os_c+1], vcc
        s_waitcnt lgkmcnt(0)
        global_store_dwordx4 v[v_tmp:v_tmp+1], v[v_c+4*.cid+20:v_c+4*.cid+23] off
        s_barrier
        .cnt = .cnt + 1
    .endr
