                   ...
                   int stride_c)
{
    ...
    //consumer
        ...
        // C store trans layout (gfx942-style, adapted for RDNA4 wave32 WMMA)
        // WMMA with src0=B(N-dim), src1=A(M-dim) outputs D[n][m] = B×A^T:
        //   lane_id % 16 → n index (N/column of C_gemm)
        //   lane_id / 16 → m group (0→m[0:7], 1→m[8:15])
        //   8 fp16/lane → 8 consecutive m values (M/row of C_gemm) at fixed n
        // Trans layout: M packed contiguously (stride 1), N stepped by stride_c
        //
        // Shape:  (MWaveGrp=2, NWaveGrp=2, NLane=16, MLane=2, MPack=8)
        // Stride: (16,          stride_c*16, stride_c, 8,       1)
