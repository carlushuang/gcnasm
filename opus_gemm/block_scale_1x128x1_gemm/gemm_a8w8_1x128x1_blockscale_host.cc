...
__global__ void gemm_a8w8_1x128x1_blockscale_kernel(opus_gemm_kargs kargs);
...
using GemmTraits = gemm_a8w8_1x128x1_blockscale_traits<>;
...
// Initialize vector with all ones for debugging.
template<typename T>
void one_vector(T* ptr, std::size_t size) {
    #pragma omp parallel for
    for (std::size_t i = 0; i < size; ++i) {
        ptr[i] = static_cast<T>(1.0f);
    }
}
    ...
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            ...
            for (int k_group_idx = 0; k_group_idx < k / group_k; ++k_group_idx) {
                ...
                const fp32_t scale_b = sfb[k_group_idx * stride_sfb + n_group];
...
void benchmark_kernel(const opus_gemm_kargs& kargs, dim3 grid, dim3 block, int warmup = 200, int iterations = 100) {
    // Allow override via env so we can crank up sampling for low-noise measurement.
    if (const char* e = std::getenv("BENCH_WARMUP"))     warmup = std::atoi(e);
    if (const char* e = std::getenv("BENCH_ITERS"))      iterations = std::atoi(e);
    int rounds = 30;
    if (const char* e = std::getenv("BENCH_ROUNDS"))     rounds = std::atoi(e);

    for (int i = 0; i < warmup; ++i) {
        gemm_a8w8_1x128x1_blockscale_kernel<Traits><<<grid, block>>>(kargs);
    ...
    // Run many measurement rounds; the BEST round (peak sustained clock) is the most
    // clock-stable signal, so use it as the primary metric. Also report median/worst.
    std::vector<fp32_t> times;
    ...
    for (int r = 0; r < rounds; ++r) {
        ...
        times.push_back(total_time / iterations);
    }
    ...
    const fp32_t best_time   = times.front();
    const fp32_t median_time = times[times.size() / 2];
    const fp32_t worst_time  = times.back();
    auto tflops = [&](fp32_t t) { return static_cast<fp32_t>(flop) / 1.0e9f / t; };

    std::printf("Kernel Performance: avg_time=%.4f ms, %.2f TFlops\n", best_time, tflops(best_time));
    std::printf("  [rounds=%d iters=%d] best=%.2f  median=%.2f  worst=%.2f TFlops  (best_ms=%.4f median_ms=%.4f)\n",
                rounds, iterations, tflops(best_time), tflops(median_time), tflops(worst_time),
...
int main(int argc, char** argv) {
    ...
    // one_vector(host_sfa.get(), sfa_count);
    rand_vector(host_sfb.get(), sfb_count, 0.8f, 1.2f);
    // one_vector(host_sfb.get(), sfb_count);
    ...
    gemm_a8w8_1x128x1_blockscale_kernel<GemmTraits><<<grid, block>>>(kargs);
    CHECK_HIP_KERNEL_LAUNCH();
