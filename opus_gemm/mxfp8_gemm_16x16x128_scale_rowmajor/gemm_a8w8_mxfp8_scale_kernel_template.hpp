...
__device__ inline unsigned int gather_broadcast_two(unsigned int raw, int span) {
    if (span == 0) return __builtin_amdgcn_mov_dpp(raw, 0xa0, 0xf, 0xf, true);
    return __builtin_amdgcn_mov_dpp(raw, 0xf5, 0xf, 0xf, true);
}
__device__ inline unsigned int prepare_gather_low2(
    unsigned int row0, unsigned int row1, int lane, int k_span) {
    const auto a = gather_broadcast_two(row0, k_span);
    const auto b = gather_broadcast_two(row1, k_span);
    const auto pair = __builtin_amdgcn_perm(b, a,
        0x06020400u + (lane & 1) * 0x01010101u);
    const auto halves = __builtin_amdgcn_permlane32_swap(pair, pair, false, true);
    return __builtin_amdgcn_perm(halves[1], halves[0],
        0x05040100u + (lane >> 5) * 0x02020202u);
}
...
// Global scales stay row-major [rows, K/32]. Adjacent lane pairs read
// consecutive 16-byte spans of one row. lane>>5 selects a pair of scale
// calls; two loads fetch its even and odd calls into two four-dword row spans.
// The pair jointly covers eight K128 tiles without an LDS queue allocation.
// call_row_stride is (T_M*W_M or T_N*W_N)*stride_sf for A or B respectively.
template<class T>
__device__ inline auto make_layout_gsf_scale(
    int lane_id, int call_row_stride, int stride_sf) {
    constexpr auto shape = opus::make_tuple(
        opus::number<T::SCALE_M_CALLS / 2>{},
        opus::number<T::W_M>{}, opus::number<2>{},
        opus::number<T::VEC_GLOBAL_SCALE>{});
    constexpr auto dims = opus::make_tuple(
        opus::make_tuple(opus::p_dim{}), opus::make_tuple(opus::p_dim{}),
        opus::make_tuple(opus::p_dim{}), opus::make_tuple(opus::y_dim{}));
    return opus::make_layout<T::VEC_GLOBAL_SCALE>(
        shape, opus::unfold_x_stride(dims, shape, opus::tuple{
            call_row_stride * 2, stride_sf,
            opus::number<T::VEC_GLOBAL_SCALE>{}, 1_I}),
        opus::unfold_p_coord(dims, opus::tuple{lane_id / (T::W_M * 2), (lane_id / 2) % T::W_M, lane_id % 2}));
}

// Each producer packs four calls for row=(lane>>1)&15 and
// kg=(lane&1)|((lane>>5)<<1). Store into the corresponding consumer slot
// kg*16+row. Consumers use contiguous lane-order dwords to avoid scale LDS
// bank conflicts; producer and consumer lane indices are different.
template<class T>
__device__ inline auto make_layout_ssf_scale(int lane_id, int wave_id_m) {
    // The low2 gather prepares row=(lane>>1)&15 and
    // kg=(lane&1)|((lane>>5)<<1). Publish in the matching consumer lane.
    const int row = (lane_id >> 1) & 15;
    const int kg = (lane_id & 1) | ((lane_id >> 5) << 1);
    const int consumer_lane = kg * 16 + row;
    ...
    return opus::make_layout<T::SCALE_M_CALLS>(shape,
        ...
        opus::unfold_p_coord(dims, opus::tuple{wave_id_m, consumer_lane}));
}
...
void gemm_a8w8_mxfp8_scale_kernel(opus_gemm_scale_kargs kargs) {
    ...
    // All eight waves are scale producers.  The four wave-N=0 waves own SFA,
    // the four wave-N=1 waves own SFB.  Both tensors stay in their logical
    // row-major [rows, K/32] layout in memory; the consumer-major image is
    // built in registers on the way into LDS.  One selected descriptor per
    // wave keeps a single scale address family live.
    ...
    // gfx950 bounds-checks each DWORD against voffset + soffset. Build
    // the pointer and its remaining-batch extent in the same uniform branch;
    // out-of-batch DWORDs in unused queue spans are zeroed; valid DWORDs remain in range.
    const D_SF* p_sf;
    unsigned int scale_bytes_remaining;
    if (scale_producer_is_sfa) {
        const int row_offset = first_block_m * T::B_M * kargs.stride_sfa;
        ...
        const int row_offset = block_n * T::B_N * kargs.stride_sfb;
    ...
    // Two four-dword row spans; lane pairs jointly supply eight K128 tiles.
    opus::u32x4_t v_scale_q0;
    opus::u32x4_t v_scale_q1;
    ...
    // One wave-uniform scale descriptor serves A or B. u_gsf distributes
    // row/call pairs and contiguous K spans across lanes; u_ssf maps each
    // prepared word into the linear consumer layout.
    const int scale_row_stride =
        scale_producer_is_sfa ? kargs.stride_sfa : kargs.stride_sfb;
    const int scale_call_row_stride =
        (scale_producer_is_sfa ? T::T_M * T::W_M : T::T_N * T::W_N) *
        scale_row_stride;
    auto u_gsf =
        make_layout_gsf_scale<T>(lane_id, scale_call_row_stride, scale_row_stride);
    ...
    // Each refill issues two b128 loads. A lane pair reads 32 contiguous
    // bytes per logical row; q0/q1 hold the even/odd call of its call pair.
    // Lane-dependent row and K-span offsets stay in voffset. The output,
    // producer wave and eight-tile group base stay uniform in soffset.
    auto gsf_offset = [&](int output_tile_index, int group_base_tile) {
        int local_role = wave_id_n;
        asm volatile("" : "+s"(local_role) ::);

        if (local_role == 0) {
            return (output_tile_index * T::B_M + wave_id_m * T::W_M) *
                       kargs.stride_sfa +
                   group_base_tile * T::NUM_KGROUPS;
        }
        return ((wave_id_m >> 1) * T::HALF_B_N + (wave_id_m & 1) * T::W_N) *
                   kargs.stride_sfb +
               group_base_tile * T::NUM_KGROUPS;
    };

    auto load_scale_row = [&](int call, int output_tile_index, int group_base_tile) {
        return __builtin_bit_cast(opus::u32x4_t,
            load<T::VEC_GLOBAL_SCALE>(g_sf, u_gsf,
                gsf_offset(output_tile_index, group_base_tile) + call * scale_call_row_stride));
    };
    auto load_scale_group = [&](int output_tile_index, int group_base_tile) {
        v_scale_q0 = load_scale_row(0, output_tile_index, group_base_tile);
        v_scale_q1 = load_scale_row(1, output_tile_index, group_base_tile);
    };

    // Fused packed-word preparation for the prologue and output handoff:
    // two quad DPP broadcasts, two byte permutations and one permlane32 swap.
    auto prepare_scale_tile = [&](int k_tile) {
        const int word = k_tile & 3;
        const int span = (k_tile >> 2) & 1;
        return prepare_gather_low2(v_scale_q0[word], v_scale_q1[word], lane_id, span);
    };

    auto store_scale_dword = [&](int write_stage, D_SF_PACK prepared) {
        int local_role = wave_id_n;
        asm volatile("" : "+s"(local_role) ::);

        const auto packed =
            __builtin_bit_cast(opus::vector_t<D_SF, T::VEC_LDS_SCALE>, prepared);
        if (local_role == 0) {
            store<T::VEC_LDS_SCALE>(s_sfa, packed, u_ssf + ssfa_offset(write_stage));
        } else {
    ...
    auto publish_scale_dword = [&](int write_stage, int k_tile) {
        store_scale_dword(write_stage, prepare_scale_tile(k_tile));
    };
    ...
    // Prologue
    if (output_tile == 0) {
        load_scale_group(output_tile, 0);
...
#pragma unroll 8
    for (tile = 0; tile + 1 < loops; ++tile) {
        ...
        // Prepare tile t+1 early, then publish it once tile t's operands are
        // resident in registers. Consume each row span separately and
        // start its future VMEM as soon as that span has become dead.
        D_SF_PACK v_scale_prepared;
        D_SF_PACK v_scale_broadcast0;
        auto transpose_next_scale_first = [&]() {
            const int word = (tile + 1) & 3;
            const int span = ((tile + 1) >> 2) & 1;
            v_scale_broadcast0 = gather_broadcast_two(v_scale_q0[word], span);
        };
        auto transpose_next_scale_rest = [&]() {
            const int word = (tile + 1) & 3;
            const int span = ((tile + 1) >> 2) & 1;
            const auto b = gather_broadcast_two(v_scale_q1[word], span);
            const auto pair = __builtin_amdgcn_perm(b, v_scale_broadcast0,
                0x06020400u + (lane_id & 1) * 0x01010101u);
            const auto halves = __builtin_amdgcn_permlane32_swap(pair, pair, false, true);
            v_scale_prepared = __builtin_amdgcn_perm(halves[1], halves[0],
                0x05040100u + (lane_id >> 5) * 0x02020202u);
        };
        auto store_next_scale = [&]() {
            store_scale_dword(next_stage, v_scale_prepared);
        };

        // Each old row span is dead after its final broadcast at t%8==6.
        // Both future requests follow the required A copies and no further
        // VMEM is issued before publication; vmcnt(2) may leave only these
        // two future scale requests outstanding. The K-tail guard is shared.
        auto refill_scale_q0 = [&]() {
            if ((tile & 7) == 6 && tile + 2 < loops) {
                asm volatile("" : "+v"(v_scale_q0), "+v"(v_scale_broadcast0) :: "memory");
                v_scale_q0 = load_scale_row(0, output_tile, tile + 2);
                __builtin_amdgcn_sched_barrier(0);
            }
        };
        auto refill_scale_q1 = [&]() {
            if ((tile & 7) == 6 && tile + 2 < loops) {
                asm volatile("" : "+v"(v_scale_q1), "+v"(v_scale_prepared) :: "memory");
                v_scale_q1 = load_scale_row(1, output_tile, tile + 2);
                __builtin_amdgcn_sched_barrier(0);
            }
        };
        ...
        // Save q0's old broadcast early. Ordinary iterations leave this VALU
        // chain free to overlap XDL; a refill iteration starts future q0 VMEM
        // here and pins it after the required A copies.
        transpose_next_scale_first();
        refill_scale_q0();
        ...
        // Finish q1's old value after six intervening MFMAs, then replace
        // its complete row span. Only the final packed word remains live.
        transpose_next_scale_rest();
        refill_scale_q1();
        ...
        //
        // The ds_write goes here rather than next to its transpose: the value
        // has been sitting ready in a VGPR since q1 was consumed above, and this
        // is the last point before the barrier that consumes it.
        store_next_scale();
        __builtin_amdgcn_sched_barrier(0);
        // Relax the wait only when the future scale load was actually issued.
        // The K-tail guard is essential: without a refill, the last request
        // could be an A copy required by tile t+1. Keep this condition matched
        // to both split refills, including in the remainder loop.
        if ((tile & 7) == 6 && tile + 2 < loops) {
    ...
    auto output_b1_handoff = [&]() {
        if (has_next_output) {
            ...
            publish_scale_dword(next_output_stage, 0);
            s_waitcnt_lgkmcnt(0_I);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
            if (loops > 1) {
    ...
    if (has_next_output) {
        ...
        load_scale_group(output_tile + 1, 0);
